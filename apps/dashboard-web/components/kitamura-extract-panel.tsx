...
export function KitamuraExtractPanel() {
  ...
  async function refresh() {
    const [statusRes, historyRes] = await Promise.all([
      fetch(`${API_BASE}/api/extract/kitamura/status`, { cache: "no-store" }),
      fetch(`${API_BASE}/api/extract/kitamura/history`, { cache: "no-store" }),
    ]);
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/extract/kitamura/start`, {
        method: "POST",
  ...
  async function onDelete() {
    ...
    setError("");
    try {
      const res = await fetch(`${API_BASE}/api/extract/kitamura/history/${selectedHistory.filename}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) {
      ...
      <form className="extract-form" onSubmit={onSubmit}>
        <label className="extract-field">
          ...
          <input
            type="url"
            required
            placeholder="https://shop.kitamura.jp/ec/list?..."
            value={searchUrl}
