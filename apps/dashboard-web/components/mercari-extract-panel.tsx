...
export function MercariExtractPanel() {
  ...
  async function refresh() {
    const [statusRes, historyRes] = await Promise.all([
      fetch(`${API_BASE}/api/extract/mercari/status`, { cache: "no-store" }),
      fetch(`${API_BASE}/api/extract/mercari/history`, { cache: "no-store" }),
    ]);
    const statusData: StatusResponse = await statusRes.json();
    const historyData: HistoryResponse = await historyRes.json();
    setActiveJob(statusData.active_job);
    setHistory(historyData.items ?? []);
    const nextSelected = selectedFile && historyData.items?.some((row) => row.filename === selectedFile)
      ? selectedFile
  ...
  const selectedHistory = useMemo(() => history.find((row) => row.filename === selectedFile) ?? null, [history, selectedFile]);
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/extract/mercari/start`, {
        method: "POST",
  ...
  async function onDelete() {
    ...
    setError("");
    try {
      const res = await fetch(`${API_BASE}/api/extract/mercari/history/${selectedHistory.filename}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) {
  ...
  async function onStop() {
    ...
    setError("");
    try {
      const res = await fetch(`${API_BASE}/api/extract/mercari/stop`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) {
  ...
  return (
    <article className="panel extract-panel">
      ...
      <form className="extract-form" onSubmit={onSubmit}>
        <label className="extract-field">
          ...
          <input
            type="url"
            required
            placeholder="https://jp.mercari.com/search?search_condition_id=..."
            value={searchUrl}
        ...
        <div className="extract-row">
          <label className="extract-field">
            <span>最大ページ数</span>
            <input type="number" min="0" max="100" placeholder="未設定で上限なし" value={maxPages} onChange={(e) => setMaxPages(e.target.value)} />
          </label>

          <label className="extract-field">
            <span>最大件数</span>
            <input type="number" min="1" max="10000" value={maxItems} onChange={(e) => setMaxItems(e.target.value)} />
          </label>
        </div>
      ...
      <div className="extract-history">
        ...
        <label className="extract-field">
          <span>履歴ファイル</span>
          <select value={selectedFile} onChange={(e) => setSelectedFile(e.target.value)} disabled={history.length === 0}>
          ...
          <div className="extract-result">
            ...
            <div className="extract-actions">
              {selectedHistory.download_url ? (
                <a className="extract-link-button" href={`${API_BASE}${selectedHistory.download_url}`} target="_blank" rel="noreferrer">
                  CSVをダウンロード
                </a>
              ) : null}
              <button className="extract-delete-button" type="button" onClick={onDelete} disabled={deleting || selectedHistory.status === "running"}>
                {deleting ? "削除中..." : "履歴を削除"}
              </button>
            </div>
          </div>
