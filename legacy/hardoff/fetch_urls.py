...
def fetch_data_from_supabase():
    ...
    while True:
        ...
        params = {
            ...
            "stocking_url": "ilike.*netmall.hardoff.co.jp*",
...
def save_filtered_csv(data):
    filtered = [row for row in data if "netmall.hardoff.co.jp" in row["stocking_url"]]

    os.makedirs("input", exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    file_path = f"input/hardoff_urls_{timestamp}.csv"
