...
from scrapers.sites.surugaya.checker import check_stock_status
...
def run_pipeline(run_id: str) -> dict:
    ...
    try:
        for row in items:
            ...
            try:
                ...
                if not target_url.startswith(('http://', 'https://')):
                    update_item_stock(ebay_item_id=ebay_item_id, scraped_stock_status='不明', is_scraped=False)
                    finish_step(step, 'success', f'invalid url skipped: {target_url}')
                    continue
                status, message = check_stock_status(driver, target_url)
                update_item_stock(ebay_item_id=ebay_item_id, scraped_stock_status=STATUS_MAP[status], is_scraped=(status != ScrapeStatus.UNKNOWN))
        ...
        try:
            ...
            pass
    return {'status': 'success', 'message': f'surugaya pipeline completed: {processed} items'}
