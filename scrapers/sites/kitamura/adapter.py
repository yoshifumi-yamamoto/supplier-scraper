...
def run_pipeline(run_id: str) -> dict:
    ...
    try:
        ...
        if not items:
            finish_step(fetch_step, status='success', message='kitamura no target items')
    ...
    return {
        ...
        'site': 'kitamura',
        'status': 'success',
        'message': f'kitamura pipeline completed: checked={processed} in={in_stock} out={out_of_stock} unknown={unknown}',
    }
