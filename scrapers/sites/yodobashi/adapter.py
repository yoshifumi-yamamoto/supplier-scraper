...
def run_pipeline(run_id: str) -> dict:
    ...
    try:
        items = fetch_active_items_by_domain(['yodobashi.com', 'www.yodobashi.com'])
        if not items:
            finish_step(fetch_step, status='success', message='yodobashi no target items')
    ...
    return {
        ...
        'site': 'yodobashi',
        'status': 'success',
        'message': f'yodobashi pipeline completed: checked={processed} in={in_stock} out={out_of_stock} unknown={unknown}',
    }
