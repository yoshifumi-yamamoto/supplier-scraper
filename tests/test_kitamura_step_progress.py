...
checker_stub = types.ModuleType("scrapers.sites.kitamura.checker")
checker_stub.check_stock_status = lambda driver, url: (_ScrapeStatus.UNKNOWN, url)
sys.modules.setdefault("scrapers.sites.kitamura.checker", checker_stub)

from scrapers.sites.kitamura import adapter as kitamura_adapter
...
class KitamuraStepProgressTests(unittest.TestCase):
    @patch("scrapers.sites.kitamura.adapter.update_item_stock")
    @patch("scrapers.sites.kitamura.adapter.check_stock_status")
    @patch("scrapers.sites.kitamura.adapter.fetch_active_items_by_domain")
    @patch("scrapers.sites.kitamura.adapter.finish_step")
    @patch("scrapers.sites.kitamura.adapter.start_step")
    @patch("scrapers.sites.kitamura.adapter.build_chrome", return_value=DummyDriver())
    def test_kitamura_uses_fetch_and_per_item_check_steps(
        self,
        _build_chrome,
        start_step_mock,
        finish_step_mock,
        ...
        fetch_mock.return_value = [
            {"ebay_item_id": "item-1", "stocking_url": "https://shop.kitamura.jp/item-1"},
            {"ebay_item_id": "item-2", "stocking_url": "https://shop.kitamura.jp/item-2"},
        ]
        check_mock.side_effect = [
            (_ScrapeStatus.IN_STOCK, "https://shop.kitamura.jp/item-1"),
            (_ScrapeStatus.OUT_OF_STOCK, "https://shop.kitamura.jp/item-2"),
        ]
        start_step_mock.side_effect = ["fetch-step", "check-step-1", "check-step-2"]

        result = kitamura_adapter.run_pipeline("run-1")
    ...
    @patch("scrapers.sites.kitamura.adapter.fetch_active_items_by_domain", return_value=[])
    @patch("scrapers.sites.kitamura.adapter.finish_step")
    @patch("scrapers.sites.kitamura.adapter.start_step", return_value="fetch-step")
    def test_kitamura_empty_fetch_uses_canonical_message(self, _start_mock, finish_step_mock, _fetch_mock) -> None:
        result = kitamura_adapter.run_pipeline("run-1")

        self.assertEqual(result["status"], "success")
        finish_step_mock.assert_called_once_with("fetch-step", status="success", message="kitamura no target items")
