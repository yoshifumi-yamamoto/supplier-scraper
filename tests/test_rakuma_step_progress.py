...
checker_stub = types.ModuleType("scrapers.sites.rakuma.checker")
checker_stub.check_stock_status = lambda driver, url: (_ScrapeStatus.UNKNOWN, "stub")
sys.modules.setdefault("scrapers.sites.rakuma.checker", checker_stub)
...
class RakumaStepProgressTests(unittest.TestCase):
    @patch("scrapers.sites.rakuma.adapter.update_item_stock")
    @patch("scrapers.sites.rakuma.adapter.check_stock_status")
    @patch("scrapers.sites.rakuma.adapter.fetch_active_items_by_domain")
    @patch("scrapers.sites.rakuma.adapter.finish_step")
    @patch("scrapers.sites.rakuma.adapter.start_step")
    @patch("scrapers.sites.rakuma.adapter.build_chrome", return_value=DummyDriver())
    def test_rakuma_uses_fetch_and_per_item_check_steps(
        self,
        _build_chrome,
        start_step_mock,
        finish_step_mock,
        ...
        fetch_mock.return_value = [
            {"ebay_item_id": "item-1", "stocking_url": "https://fril.jp/item-1"},
            {"ebay_item_id": "item-2", "stocking_url": "https://item.fril.jp/item-2"},
        ]
    ...
    @patch("scrapers.sites.rakuma.adapter.finish_step")
    @patch("scrapers.sites.rakuma.adapter.start_step", return_value="fetch-step")
    def test_rakuma_empty_fetch_uses_canonical_message(self, _start_mock, finish_step_mock, _fetch_mock) -> None:
        result = rakuma_adapter.run_pipeline("run-1")

        self.assertEqual(result["status"], "success")
        finish_step_mock.assert_called_once_with("fetch-step", status="success", message="rakuma no target items")
