...
checker_stub = types.ModuleType("scrapers.sites.yodobashi.checker")
checker_stub.check_stock_status = lambda driver, url: (_ScrapeStatus.UNKNOWN, url)
sys.modules.setdefault("scrapers.sites.yodobashi.checker", checker_stub)
...
class YodobashiStepProgressTests(unittest.TestCase):
    @patch("scrapers.sites.yodobashi.adapter.update_item_stock")
    @patch("scrapers.sites.yodobashi.adapter.check_stock_status")
    @patch("scrapers.sites.yodobashi.adapter.fetch_active_items_by_domain")
    @patch("scrapers.sites.yodobashi.adapter.finish_step")
    @patch("scrapers.sites.yodobashi.adapter.start_step")
    @patch("scrapers.sites.yodobashi.adapter.build_chrome", return_value=DummyDriver())
    def test_yodobashi_uses_fetch_and_per_item_check_steps(
        self,
        _build_chrome,
        start_step_mock,
        finish_step_mock,
        ...
        fetch_mock.return_value = [
            {"ebay_item_id": "item-1", "stocking_url": "https://www.yodobashi.com/item-1"},
            {"ebay_item_id": "item-2", "stocking_url": "https://www.yodobashi.com/item-2"},
        ]
        check_mock.side_effect = [
            (_ScrapeStatus.IN_STOCK, "https://www.yodobashi.com/item-1"),
            (_ScrapeStatus.OUT_OF_STOCK, "https://www.yodobashi.com/item-2"),
        ]
    ...
    @patch("scrapers.sites.yodobashi.adapter.finish_step")
    @patch("scrapers.sites.yodobashi.adapter.start_step", return_value="fetch-step")
    def test_yodobashi_empty_fetch_uses_canonical_message(self, _start_mock, finish_step_mock, _fetch_mock) -> None:
        result = yodobashi_adapter.run_pipeline("run-1")

        self.assertEqual(result["status"], "success")
        finish_step_mock.assert_called_once_with("fetch-step", status="success", message="yodobashi no target items")
